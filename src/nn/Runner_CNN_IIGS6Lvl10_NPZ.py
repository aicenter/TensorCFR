...
class Runner_CNN_IIGS6Lvl10_NPZ:
	@staticmethod
	def parse_arguments():
		import argparse
	...
	def run_neural_net():
		...
		args = Runner_CNN_IIGS6Lvl10_NPZ.parse_arguments()
		dataset_directory = args.dataset_directory
		args = Runner_CNN_IIGS6Lvl10_NPZ.create_logdir(args)

		devset, testset, trainset = Runner_CNN_IIGS6Lvl10_NPZ.init_datasets(dataset_directory)
		network = Runner_CNN_IIGS6Lvl10_NPZ.construct_network(args)

		for epoch in range(args.epochs):
			Runner_CNN_IIGS6Lvl10_NPZ.train_one_epoch(args, network, trainset)
			Runner_CNN_IIGS6Lvl10_NPZ.evaluate_devset(devset, epoch, network)

		Runner_CNN_IIGS6Lvl10_NPZ.evaluate_testset(network, testset)
		Runner_CNN_IIGS6Lvl10_NPZ.showcase_predictions(network, trainset)
