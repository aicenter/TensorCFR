...
reach_probabilities_lvl_1 = reach_probabilities_without_normalization \
...
strategies = tf.reshape(tf.range(0, 27), shape3x3x3)
# contribution of strategies to the counterfactual probability
strategies_lvl_2 = tf.where(mask_actions, tf.ones_like(strategies), strategies)

with tf.Session() as sess:
	sess.run(tf.global_variables_initializer())
	# print("reach_probabilities_lvl_1:\n", sess.run(reach_probabilities_lvl_1))
	print("acting_players:\n", sess.run(acting_players))
	# print("mask_players:\n", sess.run(mask_players))
	# print("mask_actions:\n", sess.run(mask_actions))
	print("strategies:\n", sess.run(strategies))
	print("strategies_lvl_2:\n", sess.run(strategies_lvl_2))
