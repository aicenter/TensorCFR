...
from src.utils.cfr_utils import flatten_strategies_via_action_counts, get_action_and_infoset_values, \
	distribute_strategies_to_inner_nodes, distribute_range_strategies_to_nodes,distribute_range_strategies_to_inner_nodes
...
class TensorCFRFixedTrunkStrategies:
	...
	def get_node_range_strategies(self, for_player=None):
		if for_player is None:
			for_player = self.domain.current_updating_player
		with tf.variable_scope("nodal_range_strategies_p{}".format(for_player)):
			# TODO generate node_cf_strategies_* with tf.where on node_strategies
			node_range_strategies = [
				distribute_range_strategies_to_inner_nodes(
					self.domain.current_infoset_strategies[level],
					self.domain.node_to_infoset[level],
					self.domain.mask_of_inner_nodes[level],
					for_player=for_player,
					acting_players=self.domain.infoset_acting_players[level],
					name="node_range_strategies_p{}_lvl{}".format(for_player,level)
				) for level in range(self.acting_depth)
			]
			flattened_node_range_strategies = flatten_strategies_via_action_counts(
				node_range_strategies,
				self.action_counts,
				basename="nodal_range_strategies_p{}".format(for_player)
			)
	...
	def get_nodal_cf_values_at_trunk_depth(self, for_player=None):  # TODO verify and write a unittest
		"""
		Compute counterfactual values of nodes by (tensor-)multiplying reach probabilities and expected values.
		...
		reach_probabilities = self.get_nodal_reach_probabilities(for_player=for_player)
		with tf.variable_scope("nodal_counterfactual_values"):
			return tf.multiply(reach_probabilities[self.trunk_depth],
					expected_values[self.trunk_depth],
					name="nodal_cf_value_lvl{}".format(self.trunk_depth))
	...
	def get_infoset_ranges_at_level(self,level=None):
		...
		"""
		ranges = {}
		if self.trunk_depth and level > 0:
			ranges = {}
			for player in [PLAYER1, PLAYER2]:
				opponent = PLAYER2 if player == PLAYER1 else PLAYER1
				range_of_the_player = self.get_infoset_reach_probabilities(for_player=opponent)
				ranges[player] = tf.expand_dims(
					range_of_the_player[level],
					axis=-1
				)
	...
	def get_trunk_info_of_nodes_cfv_sep_reaches(self):
		...
		if self.concat_trunk_info_tensors is None:
			...
			inner_nodal_reaches_for_player0 = tf.expand_dims(self.get_separate_nodal_reach_probabilities()[0],
			                                                 axis=-1,
			                                                 name="inner_nodal_reaches_p1_lvl{}".format(self.boundary_level))
			print("chance reach prob shape:{}".format(inner_nodal_reaches_for_player0.shape))

			inner_nodal_reaches_for_player1 = tf.expand_dims(self.get_separate_nodal_reach_probabilities()[1],
			...
			inner_nodal_reaches_for_player2 = tf.expand_dims(self.get_separate_nodal_reach_probabilities()[2],
			...
			inner_nodal_reaches_for_player_total = tf.expand_dims(self.get_separate_nodal_reach_probabilities()[3],
			...
			inner_nodal_range_for_player0 = tf.expand_dims(self.get_nodal_range_probabilities(for_player=0),
			                                               axis=-1,
			                                               name="range_p0_lvl{}".format(self.boundary_level))
			print("p20 reach prob shape:{}".format(inner_nodal_range_for_player0.shape))


			inner_nodal_range_for_player1 = tf.expand_dims(self.get_nodal_range_probabilities(for_player=1),
			                                                 axis=-1,
			                                                 name="range_p1_lvl{}".format(self.boundary_level))
			print("p1 reach prob shape:{}".format(inner_nodal_range_for_player1.shape))

			inner_nodal_range_for_player2 = tf.expand_dims(self.get_nodal_range_probabilities(for_player=2),
			                                               axis=-1,
			                                               name="range_p2_lvl{}".format(self.boundary_level))
			...
			self.concat_trunk_info_tensors = tf.concat(
				[
					...
					inner_nodal_range_for_player0,
					inner_nodal_range_for_player1,
					inner_nodal_range_for_player2,
					inner_nodal_expected_values
				],
				axis=-1,
				name="concat_trunk_info_tensors_lvl{}".format(self.boundary_level)
	...
	def store_trunk_info_of_nodes_cf_sep_reach(self, dataset_basename, dataset_directory=""):
		self.session.run(self.assign_avg_strategies_to_current_strategies())
		...
		csv_file = open(csv_filename, 'ab')  # binary mode for appending
		trunk_info_of_nodes_cfv_sep_reaches = self.get_trunk_info_of_nodes_cfv_sep_reaches()

		data_to_store = self.session.run(trunk_info_of_nodes_cfv_sep_reaches)

		np.savetxt(
			csv_file,
			data_to_store,
			fmt="%7d,\t %7d,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f,\t %+.6f",
			header="nodal_index,\t node_to_infoset,\t nodal_reach_0,\t nodal_reach_1,\t nodal_reach_2,\t nodal_reach_total,\t nodal_reach_noacting,\t nodal_cf_value,\t reach_0,\t reach_1,\t reach_2,\t nodal_exp_value"
		)
		csv_file.close()
	...
	def generate_dataset_single_session_cf_sep_reach(self, total_steps=DEFAULT_TOTAL_STEPS, delay=DEFAULT_AVERAGING_DELAY,
	                                    dataset_for_nodes=True, dataset_size=DEFAULT_DATASET_SIZE, dataset_directory="",
	                                    dataset_seed_to_start=0):
		self.set_up_dataset_generation(delay, total_steps)
