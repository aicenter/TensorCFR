...
def get_cf_values_of_infosets_actions():  # TODO verify and write a unittest
	...
	# for level in range(levels - 1):         # TODO replace for-loop with parallel_map on TensorArray?
	for level in [1]:   # TODO use for all levels
		scatter_nd_add_ref = cf_values_of_IS[level]
		# TODO how to do the following without tf.expand_dims()
		scatter_nd_add_indices = tf.expand_dims(node_to_IS[level], axis=-1, name="IS_action_cf_values_lvl{}".format(level))
		...
		print("########## Level {} debug ##########".format(level))
		with tf.Session() as sess:
			sess.run(tf.global_variables_initializer())
			print_tensors(sess, [scatter_nd_add_ref, scatter_nd_add_indices, scatter_nd_add_updates, cf_values_of_IS[level]])
