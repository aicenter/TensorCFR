...
class GambitLoaderCached(GambitLoader):
    ...
    def _save_hkl(self, path_to_hkl):
        ...
        with h5py.File(path_to_hkl, 'w') as f:
            ...
            f.create_dataset('nodes_per_levels', dtype=np.int32, shape=nodes_per_levels_list.shape, data=nodes_per_levels_list)

            f.create_dataset('number_of_levels', dtype=np.int32, shape=(1,), data=self.number_of_levels)
            ...
            f.create_dataset('number_of_players', dtype=np.int32, shape=(1,), data=self.number_of_players)
...
if __name__ == '__main__' and activate_script():
    ...
    for efg_file, domain_name in zip(efg_files, domain_names):
        GambitLoaderCached(efg_file, domain_name)
